...
# A class can implement one or more interfaces by providing
  an implementation for each method in the interface.
...
public class InterfacesApp {
    public static void Main (string[] args) {
        ...
        entities[4] = new Enemy("Warewolf", 35, 10);

        Random random = new Random();
        int turn = 0;
        Player player = (Player) entities[0];
        int aliveEnemiesCount = entities.Length - 1;
        while (entities[0].IsAlive && aliveEnemiesCount > 0 ) {
            turn++;
            ...
            for (int i = 1; i < entities.Length; i++)
            {
                ...
                if (!enemy.IsAlive) {
                    ...
                    enemy.Log();
                    enemy.TakeDamage(random.Next(0, player.Damage));
                    if (enemy.IsAlive) {
                        player.Log();
                        player.TakeDamage(random.Next(0, enemy.Damage));
                        if (player.HP <= 0) {
                            break;
                        }
                    }
...
public class Player : GameEntity, ILogger
{
...
public class Enemy : GameEntity, ILogger
{
